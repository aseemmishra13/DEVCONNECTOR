...
router.delete('/comment/:id/:comment_id',auth, async(req,res) =>{
    ...
    try {
        ...
        const comment = post.comments.find(comment=> commentid= req.params.comment_id);

        if(!comment) return res.status(404).json({msg:'Comment does not exist'})

        if(comment.user.toString()!== req.user.id) return res.status(401).json({msg:'User isnot authorized'})
